...
for y in inscen:
    for x in incrop:
        mxd1 = arcpy.mapping.MapDocument(r"D:\\CROP_SUMMARY_SUND\\yield_sund_maps_temp.mxd")
        path=inPath +x+"*"+y+"*.tif"
        mxd1.saveACopy(outPath+x+"_"+y+".mxd")
        mxd=arcpy.mapping.MapDocument(outPath+x+"_"+y+".mxd")
        for file in glob.glob(path):
            if not "DEV" in file:
                symbologyLyr =r"D:\\CROP_SUMMARY_SUND\\symb\\symbology_"+x+".lyr"
                dnnm=file.split('\\')
                #print (dnnm[3])
        ...
        mxd.save()
        arcpy.mapping.ExportToPNG(mxd,outPath+x+"_"+y+".png")
        #time.sleep(10)
